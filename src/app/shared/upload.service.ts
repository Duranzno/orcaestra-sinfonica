...
export class UploadService {
  ...
  public uploadScore(score){
    this.db.collection('partituras').add({score})
  ...
  public upload2(files: Set<File>): { [key: string]: Observable<number> } {
    ...
    files.forEach(file => {// create a new multipart-form for every file
      ...
      let startTime = new Date().getTime();// send the http-request and subscribe for progress-updates
